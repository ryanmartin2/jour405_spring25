...
Off the bat, it's difficult to see a clear correlated relationship between the FARMS percentage and the math test scores. The red line shows visually how when the FARMS percentage increases, the math test scores don't necessarily increase with it.
...
Based on what we have learned so far using the model to compare FARMS and math test scores in this county, we can see that the model and this data between the two are not worth using. The summary we just created shows us that we can only explain/account for 2.5% of the math test scores using the FARMS percentage. This important usage of the adjusted r-squared data reveals the nature of the FARMS percentage and the math scores — showing us that the correlation here isn't high enough for them to be dependent on each other.
...
From the model, we know that there's not much correlation between FARMS percentage and math test scores. But there's still a story to be had here. The model's results can influence a story pitch about the school board member and math test scores because we can go investigate why they are incorrect to think that a relationship is there. The information here and school board member's information may be cherry-picked here or may be looking for data with a chosen thesis in mind that its correlated here. So, the story can be trying to find the context as to why they're wrong and developing away from this sole data set to see what other things are factoring into the fluctuating math test scores.
